...
public enum TossPaymentError {
    ...
    private static final EnumSet<TossPaymentError> FAILURE_ERROR = EnumSet.of(
    ...
    public static TossPaymentError toTossPaymentError(String errorCode) {
        return Arrays.stream(TossPaymentError.values())
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("코드를 찾을 수 없습니다. ErrorCode: %s".formatted(errorCode)));
    }

    public Boolean isSuccess() {
        return this.equals(ALREADY_PROCESSED_PAYMENT); // 이미 처리된 결제에 대해서만 성공
    }

    public Boolean isFailure() {
        return !FAILURE_ERROR.contains(this); // 포함하면 아직 알 수 없는 예외이기 때문에 false
    }

    public Boolean isUnknown() {
        return !isFailure() && !isSuccess(); // isFailure 에서 ALREADY_PROCESSED_PAYMENT 제외
    }
}
